...
export function VideoGrid({ participants, activeSpeakerId, localVideoRef }: VideoGridProps) {
  ...
  return (
    <div className="flex-1 flex flex-col gap-4 p-4 min-h-0 lg:gap-6 lg:p-6 pb-20 lg:pb-6">
      ...
      {participants.length === 1 && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="text-center text-zinc-400 bg-zinc-900/80 backdrop-blur-sm px-6 py-5 rounded-lg border border-zinc-800">
            <p className="text-sm lg:text-base">Estás solo en la reunión</p>
            <p className="text-xs lg:text-sm text-zinc-500 mt-2">Comparte el código de la reunión para que otros se unan</p>
          </div>
        </div>
      )}
    </div>
